...
class BusinessSettingsController extends Controller
{
    ...
    public function outlet_commission(Request $data)
    {
        $validatedData = $data->validate([
            'commission' => 'required|min:0',
        ]);
        $sales_commission = BusinessSetting::where('type', 'outlet_commission')->first();

        if (isset($sales_commission)) {
            BusinessSetting::where('type', 'outlet_commission')->update(['value' => $data->commission]);
        } else {
            DB::table('business_settings')->insert(['type' => 'outlet_commission', 'value' => $data->commission]);
        }

        Toastr::success('Outlet commission Updated successfully!');
