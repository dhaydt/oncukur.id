        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="card border-0 box-shadow">
                    <div class="card-body">
                        <h2 class="h4 mb-1">{{\App\CPU\translate('Login')}}</h2>
                        <hr class="mt-2">
                        ...
                        <form class="needs-validation mt-2" autocomplete="off" action="{{route('customer.auth.login')}}"
                            ...
                            <button class="btn btn-primary btn-block btn-shadow"
                                    type="submit">{{\App\CPU\translate('login')}}</button>
                        </form>
                    </div>
                    <div class="card-footer">
                        <div class="row">
                            <div class="col-12 flex-between row p-0" style="direction: {{ Session::get('direction') }}">
                                <div class="mb-3 d-flex align-items-center {{Session::get('direction') === "rtl" ? '' : 'ml-2'}}">
                                    <h6>{{ \App\CPU\translate('no_account') }} ? Register now</h6>
                                </div>
                                <div class="mb-3 {{Session::get('direction') === "rtl" ? 'ml-2' : ''}}">
                                    <a class="btn btn-outline-primary"
                                       href="{{route('customer.auth.register')}}">
                                        <i class="fa fa-user-circle"></i> {{\App\CPU\translate('register')}}
                                    </a>
                                </div>
                            </div>
