    ...
    <div class="content container-fluid">
                        ...
                        <div class="row col-lg mb-3 mb-lg-0 {{Session::get('direction') === "rtl" ? 'mr-2' : 'ml-2'}}" style="display: block; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                            ...
                            <div class="row align-items-center">
                                <div class="flex-between col-auto">
                                    <h5 class="text-muted {{Session::get('direction') === "rtl" ? 'ml-1' : 'mr-1'}}">{{\App\CPU\translate('Outlet')}} : </h5>
                                    <h5 class="text-muted text-capitalize">{{auth('seller')->user()->f_name}}</h5>
                                </div>
        ...
        <div class="row border-bottom border-right border-left border-top">
            ...
            @php
                ...
                $to = session('to_date');
                $total_tax=\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(),'status'=>'disburse'])
            ...
            @endphp
            @php
                $total_earning =\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(),'status'=>'disburse'])
               ->whereBetween('created_at', [$from, $to])
               ->sum('order_amount');
                if($total_earning==0){
                    $total_earning=.01;
            ...
            @endphp
            @php
                $total_commission =\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(),'status'=>'disburse'])
               ->whereBetween('created_at', [$from, $to])
        ...
        <div class="card mb-3 mb-lg-5 border-top border-left border-right border-bottom">
            ...
            <div class="card-header">
                @php
                    $total_sold=\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(),'status'=>'disburse'])->whereBetween('created_at', [date('y-01-01'), date('y-12-31')])->sum('order_amount');
                    $t=\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(),'status'=>'disburse'])->whereBetween('created_at', [date('y-01-01'), date('y-12-31')])->sum('tax');
                    $c=\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(),'status'=>'disburse'])->whereBetween('created_at', [date('y-01-01'), date('y-12-31')])->sum('admin_commission');
        ...
        @php
            $sold=[];
                for ($i=1;$i<=12;$i++){
                    $from = date('Y-'.$i.'-01');
                    $to = date('Y-'.$i.'-30');
                    $sold[$i]=\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(),'status'=>'disburse'])->whereBetween('created_at', [$from, $to])->sum('order_amount');
                }
        @endphp
        ...
        @php
            $tax=[];
                for ($i=1;$i<=12;$i++){
                    $from = date('Y-'.$i.'-01');
                    $to = date('Y-'.$i.'-30');
                    $tax[$i]=\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(), 'status'=>'disburse'])->whereBetween('created_at', [$from, $to])->sum('tax');
                }
        @endphp
        @php
            $commission=[];
                for ($i=1;$i<=12;$i++){
                    $from = date('Y-'.$i.'-01');
                    $to = date('Y-'.$i.'-30');
                    $commission[$i]=\App\Model\OrderTransaction::where(['seller_id' => auth('seller')->id(), 'status'=>'disburse'])->whereBetween('created_at', [$from, $to])->sum('admin_commission');
                }
        @endphp
